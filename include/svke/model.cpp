...
namespace svke {
  ...
  void Model::pCreateVertexBuffer(const std::vector<Vertex>& vertices) {
    ...
    pDevice.CreateBuffer(buffer_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, pVertexBuffer,
                         pVertexBufferMemory);
