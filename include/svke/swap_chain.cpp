...
namespace svke {
  ...
  VkResult SwapChain::AcquireNextImage(uint32_t *imageIndex) {
    vkWaitForFences(pDevice.getDevice(), 1, &pInFlightFences[pCurrentFrame], VK_TRUE,
                    std::numeric_limits<uint64_t>::max());

    return vkAcquireNextImageKHR(pDevice.getDevice(), pSwapChain, std::numeric_limits<uint64_t>::max(),
  ...
  void SwapChain::pCreateDepthResources() {
    ...
    for (uint64_t i = 0; i < pDepthImages.size(); i++) {
      ...
      pDevice.CreateImageWithInfo(image_info, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, pDepthImages[i],
                                  pDepthImageMemorys[i]);
